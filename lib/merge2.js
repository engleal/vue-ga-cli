...
function diffFile(localObj, templateObj, ancestorsObj) {
  ...
  templateList.forEach((filePath, index) => {
    ...
    if (!localObj[filePath]) {
      ...
      // 目标分支 也就是本地项目的内容
      const targetBranch = localFileContent
      // merge分支 也就是网上的模板项目的内容
      const mergeBranch = templateFileContent
      getDiffContent(ancestorsBranch, targetBranch, mergeBranch, localFilePath)
    }
  })
  spinner.succeed()
...
function getDiffContent(
  ...
  const nodeDiffResult =
    Diff3.merge(ancestorsBranch, targetBranch, mergeBranch, {
      stringSeparator: /((\r\n)|\n|\r)+/,
    }) || []
  let content = ''
  ...
  if (localFilePath.indexOf("package.json") !== -1) {
    console.log("targetBranch", targetBranch)
    console.log("mergeBranch", mergeBranch)
    console.log("ancestorsBranch", ancestorsBranch)
