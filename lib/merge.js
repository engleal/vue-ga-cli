...
function diffFile(localObj, templateObj, ancestorsObj, tags) {
  ...
  templateList.forEach((filePath, index) => {
    ...
    if (!localObj[filePath]) {
      let content = readFileContent(templateObj[filePath])
      crateFile(filePath, content)
    } else {
      const localFilePath = localObj[filePath]
      // 如果在业务子系统随意更改templateInfo中的tags将导致bug 请勿随意更改tags 随意更改将导致这个path为undefined将导致问题
      ...
      if (isMediaSource(filePath)) {
        ...
        const ancestorsFileContent = ancestorsFilePath ? readFileContent(ancestorsFilePath):""
...
function getDiffContent(
  ...
  nodeDiffResult.result.map((row, index) => {
    if(filePath.indexOf(".browserslistrc") !== -1){
      console.log("row",row)
    }
    if (/((>>>){3,8}|(<<<){3,8}|(===){3,8})/.test(row)) {
      if (/((>>>){8}|(<<<){8}|(===){8})/.test(row)) {
        content += `${row}${newLineSymbol}`
      } else {
        if (row.indexOf('<') !== -1) {
          content += `<<<<<<<<<${newLineSymbol}`
...
function runCommandCommit(oldUrl, ancestorUrl, cb) {
  runCommand('commit', oldUrl, ancestorUrl, ['-am', '模板数据同步'], cb)
}
...
// 运行命令
function runCommand(command, oldUrl, ancestorUrl, option = [], cb) {
  ...
  childProcess.on('close', function (status) {
    console.log(`${command}结果：`, `${status === 0 ? '成功' : '发生异常'}\r\n`)
    if (status == 0) {
      cb && cb()
    } else {
